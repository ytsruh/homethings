    ...
    case "GET":
      try {
        const books: Book[] = await db
          .select()
          .from(book)
          .where(and(eq(book.userId, token.id), eq(book.read, false)))
...
export const getUnreadBooks = async (ctx: GetServerSidePropsContext) => {
  const token = await getToken({ req: ctx.req });
  try {
    const books: Book[] = await db
      .select()
      .from(book)
      .where(and(eq(book.userId, token?.sub as string), eq(book.read, false)))
      .orderBy(asc(book.name));
