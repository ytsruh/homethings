...
export async function POST(req: NextRequest) {
  try {
    ...
    const data: User[] = await db.select().from(users).where(eq(users.email, body.email));
    ...
    const match = await bcrypt.compare(body.password as string, foundUser!.password as string);
